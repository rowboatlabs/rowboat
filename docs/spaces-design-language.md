...
10. The stream composer's Auto toggle (2026-09-22) hands the destination to Jev, TypeSafe's System One model: at send time the draft is a new stream message or a reply to the recent root or open thread it continues (a root with no replies is a thread too). Two modes (2026-09-23): Preview, the default whenever Auto is turned on, never posts on a first send. A thread pick opens that thread with the reply staged in its composer, joined to any draft already there; a stream verdict stays in the stream composer under a notice, and sending again on the same text, or the notice's button, posts it without asking again. The notice names the closest thread when Jev gave it real probability, as a one-click "Reply in … instead" that stages the text there (2026-09-24); a picker stands in only when no thread came close. Post holds a thread reply for five seconds with an Undo toast, then sends it and a toast names the thread; a stream verdict in Post posts at once. Off, sends go to the stream as before. Without a TypeSafe key (the Decision Models card) the pill, the /find command and the tag switch do not appear at all, like the Terminal pill without code mode, and the composer is unchanged. Thread composers have no toggle.
