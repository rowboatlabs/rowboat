...
<Note> Text RAG and local file uploads are enabled by default - no configuration needed! Just start using them right away. </Note>
...
- For larger files, we recommend using Gemini models — see below.
...
<Warning> The default size for the vectors index is 1536. If you change this value, then you must delete the index and set it up again: </Warning>
