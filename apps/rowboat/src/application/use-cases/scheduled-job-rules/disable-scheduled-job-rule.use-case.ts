...
export interface IDisableScheduledJobRuleUseCase {
    execute(request: z.infer<typeof inputSchema>): Promise<z.infer<typeof ScheduledJobRule>>;
}

export class DisableScheduledJobRuleUseCase implements IDisableScheduledJobRuleUseCase {
    ...
    async execute(request: z.infer<typeof inputSchema>): Promise<z.infer<typeof ScheduledJobRule>> {
        ...
        // disable the scheduled job rule
        const disabledRule = await this.scheduledJobRulesRepository.disable(request.ruleId);
