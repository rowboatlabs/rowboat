...
const WELCOME_CONTENT = `# Welcome to OpenClaw

This vault is your work memory.

OpenClaw extracts context from your emails and meetings and turns it into long-lived, editable Markdown notes. The goal is not to store everything, but to preserve the context that stays useful over time.
...
As new emails and meetings come in, OpenClaw adds decisions, commitments, and relevant context to the appropriate notes.
...
OpenClaw uses this shared memory to help with everyday work, such as:
...
**Reduce noise**
OpenClaw focuses on recurring contacts and active projects instead of trying to capture everything.
...
As you keep using OpenClaw, context accumulates across notes instead of being reconstructed from scratch.

---

If something feels confusing or limiting, we'd love to hear about it.
OpenClaw is still evolving, and your workflow matters.
`;
