...
12. **Meeting People notes come from the calendar invite, not the transcript.** When a recorded-meeting source carries an ATTENDEES-GATE banner, it is authoritative: new People notes ONLY for people on that list (teammates on the list included; the owner never). Transcript-only names are speech-recognition output — at most a prose mention, or an update to an EXISTING People note that clearly matches (uninvited joiners happen); never a new note. No attendee list → no new People notes from that meeting.
...
- Someone not on the invite may still have joined the meeting. If an EXISTING People note clearly matches a transcript name (name/alias plus context), update that note with the meeting activity — but never CREATE a new note for anyone off the list.
- A name with no plausible attendee match and no clearly matching existing note is a transcription artifact or a person merely mentioned: it may appear in prose (summaries, activity lines), but it never becomes a note and never becomes an alias of an unrelated person.
