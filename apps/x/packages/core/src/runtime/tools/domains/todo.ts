...
export const todoTools: z.infer<typeof BuiltinToolsSchema> = {
    'todo-report': {
        ...
        execute: async ({ item, status, summary, links }: {
            item: string;
            ...
            try {
                ...
                const { attachReceipt } = await import("../../../todo/fileops.js");
                ...
                const attached = await attachReceipt(item, receipt, { check: status === 'done' });
