...
No chip is set, but code mode is enabled (this skill only loads when it is). **Proceed to Step 2 with agent = \`claude\` — do NOT ask.** Coding requests dispatch immediately; a question here is friction, especially on voice. Mention the choice in your one-line narration ("Using Claude Code — toggle the composer's code chip for another agent") and move on. Only if this conversation's earlier coding turns used \`codex\` or \`opencode\`, stay with that same agent.
