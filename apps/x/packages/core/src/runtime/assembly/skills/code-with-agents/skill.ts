...
All coding work runs through the **\`code_agent_run\`** tool. It launches the selected on-device coding agent (OpenCode, Cursor, or Hermes), streams its tool calls, file diffs, and plan into the chat, and surfaces any action needing approval as an inline permission card. One persistent session is kept per chat, so follow-up requests resume with full context automatically.
...
No chip is set, but code mode is enabled (this skill only loads when it is). **Proceed to Step 2 with agent = \`claude\` — do NOT ask.** Coding requests dispatch immediately; a question here is friction, especially on voice. Mention the choice in your one-line narration ("Using OpenCode — toggle the composer's code chip for another agent") and move on. Only if this conversation's earlier coding turns used \`codex\` or \`opencode\`, stay with that same agent.
...
**Pick the agent** (\`claude\`, \`codex\`, or \`opencode\`): use the agent from the "# Code Mode (Active)" block (the composer chip) / the Step 1 choice. The chip is authoritative — do NOT carry over a different agent from earlier in this thread, and do NOT switch on an in-chat text request ("use codex"); tell the user to toggle the chip instead.
...
> Using [OpenCode / Cursor / Hermes] to [task description] in \`[folder]\` — or "in your default repo" when cwd is omitted.
...
- ONE exception: when the user explicitly asks you to gather outside context first ("fetch the error from my email and send it to OpenCode", "pull the spec from my knowledge base for Cursor"), collect that context, then send their verbatim request followed by the gathered material under a clearly labeled section (e.g. "Context the user asked me to include:").
...
- **Reply in ~2 lines, nothing more.** The agent's ENTIRE output — every message, plan step, tool call, and file diff — is directly visible to the user, fully formatted, in the run card right above your reply; the user reads the results THERE. On success your reply is exactly: (1) one line stating what was done — "I used OpenCode to implement [task]." — and (2) at most one more line with a key outcome the user needs, when applicable: a PR link (if the run created one), or the changes' status / next step.
...
- Hermes — https://github.com/duketopceo/hermes
`;
