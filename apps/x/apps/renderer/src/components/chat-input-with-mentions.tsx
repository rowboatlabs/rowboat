...
import { AGENT_CATALOG, KNOWN_AGENTS, agentLabel } from '@x/shared/src/agent-catalog.js'
import { fetchCodeAgentsStatus, isAgentReady, type CodeAgentsStatus } from './code/code-agent-status'
...
function ChatInputInner({
  ...
  // Readiness drives which agents the chip may land on: cycling into an agent
  // that isn't installed would fail at run time.
  useEffect(() => {
  ...
  const loadCodingAgentFor = useCallback(async (dir: string | null): Promise<CodingAgent> => {
    ...
    try {
      ...
      if (typeof value === 'string' && (KNOWN_AGENTS as readonly string[]).includes(value)) return value as CodingAgent
    } catch {
  ...
  const persistCodingAgent = useCallback(async (dir: string, agent: CodingAgent) => {
    ...
    try {
      ...
      for (const [k, v] of Object.entries(parsed ?? {})) {
        if (typeof v === 'string' && (KNOWN_AGENTS as readonly string[]).includes(v)) existing[k] = v as CodingAgent
      }
    } catch { /* start fresh */ }
    existing[dir] = agent
    await window.ipc.invoke('workspace:writeFile', {
  ...
  // If the current/persisted selection isn't ready but another agent is, move to
  // it. This is what makes "only OpenCode installed" work: the default is not
  // hardcoded to Claude Code.
  useEffect(() => {
    if (!agentsStatus || isCodeLocked) return
    if (isAgentReady(agentsStatus, codingAgent)) return
    const firstReady = KNOWN_AGENTS.find((a) => isAgentReady(agentsStatus, a))
    if (firstReady) setCodingAgent(firstReady)
  }, [agentsStatus, codingAgent, isCodeLocked])
  ...
  const handleToggleCodingAgent = useCallback(async () => {
    if (isCodeLocked) return
    const ready = agentsStatus ? KNOWN_AGENTS.filter((a) => isAgentReady(agentsStatus, a)) : []
    const list = ready.length > 0 ? ready : KNOWN_AGENTS
    const idx = list.indexOf(codingAgent)
    const next: CodingAgent = list[(idx + 1) % list.length]
    setCodingAgent(next)
  ...
  }, [workDir, codingAgent, persistCodingAgent, isCodeLocked, agentsStatus])
        ...
        {codeModeFeatureEnabled && collapseLevel < 5 && ((isCodeLocked || codeModeEnabled) ? (
          collapseLevel >= 1 ? (
            ...
            <Tooltip delayDuration={CHAT_INPUT_TOOLTIP_DELAY_MS}>
              ...
              <TooltipContent side="top">
                {isCodeLocked
                  ? `Coding session — ${agentLabel(codingAgent)}`
                  : `Code mode on (${agentLabel(codingAgent)}) — click to disable`}
              </TooltipContent>
            </Tooltip>
          ) : (
            <div className="flex h-7 shrink-0 items-center rounded-full bg-secondary text-xs font-medium text-foreground">
              ...
              <Tooltip delayDuration={CHAT_INPUT_TOOLTIP_DELAY_MS}>
                <TooltipTrigger asChild>
                  ...
                  >
                    <span>{AGENT_CATALOG[codingAgent].shortLabel}</span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="top">
                  {isCodeLocked
                    ? `Coding agent fixed by the session: ${agentLabel(codingAgent)}`
                    : `Coding agent: ${agentLabel(codingAgent)} — click to swap`}
                </TooltipContent>
              </Tooltip>
            </div>
          )
          ...
          <Tooltip delayDuration={CHAT_INPUT_TOOLTIP_DELAY_MS}>
            ...
            <TooltipContent side="top">Use a coding agent</TooltipContent>
          </Tooltip>
        ))}
        </div>
        {collapseLevel >= 5 && (
                ...
                <>
                  ...
                  {(isCodeLocked || codeModeEnabled) && (
                    <DropdownMenuItem disabled={isCodeLocked} onSelect={(e) => { e.preventDefault(); handleToggleCodingAgent() }}>
                      ...
                      <span className="text-xs text-muted-foreground">{AGENT_CATALOG[codingAgent].shortLabel}</span>
                    </DropdownMenuItem>
                  )}
                </>
              )}
