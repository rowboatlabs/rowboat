...
export function MessageLinkPreview({ body }: { body: string }) {
    ...
    return (
        <div className="mt-1 flex max-w-md items-start gap-3 rounded-lg border-l-4 border-l-[var(--codex-link)] bg-muted/40 py-2 pl-3 pr-3">
            <div className="min-w-0 flex-1">
                ...
                {preview.title && (
                    <button
                        ...
                        onClick={open}
                        title={preview.url}
                        className="block max-w-full truncate text-left text-sm font-semibold text-[var(--codex-link)] hover:underline"
                    >
                        {preview.title}
                    </button>
                )}
