...
export function AppDetail({
    ...
    runAction('update', async () => {
      try {
        ...
        if (msg.includes('new_capabilities')) {
          if (
            !window.confirm(
              `This update widens the app's access:\n${msg}\n\nProceed?`
            )
          )
            return
          await window.ipc.invoke('apps:update', {
            folder,
            confirmNewCapabilities: true,
        ...
        if (msg.includes('modified_files')) {
          if (
            !window.confirm(
              `You modified files this update will overwrite:\n${msg}\n\nProceed?`
            )
          )
            return
          await window.ipc.invoke('apps:update', {
  ...
  const doUninstall = () =>
    runAction('uninstall', async () => {
      ...
      if (
        !window.confirm(
          `Uninstall this app? Its data/ folder will be deleted.${agentNote}`
        )
      )
        return
  ...
  const doDelete = () =>
    runAction('delete', async () => {
      ...
      if (
        !window.confirm(
          `Delete this app? The whole folder, including data/, is removed from this machine.${publishNote}${agentNote}`
        )
      )
        return
  ...
  const toggleAgent = (slug: string, active: boolean) =>
    runAction(slug, async () => {
      const r = await window.ipc.invoke('bg-task:patch', {
        slug,
        partial: { active }
      })
      if (!r.success)
        throw new Error(r.error || 'Could not change automatic updates.')
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 text-sm">
        ...
        {notice && (
          <div className="mb-3 rounded-md border border-border bg-muted/40 px-3 py-2 text-muted-foreground">
            {notice}
          </div>
        )}
        {!app ? (
          ...
          <div className="space-y-5">
            <section className="space-y-2">
              ...
              {agents.length === 0 ? (
                <p className="text-muted-foreground">
                  {app.agentSlugs.length
                    ? 'Agent setup is incomplete. Ask the copilot to repair the bundled agents.'
                    : 'No background updates configured.'}
                </p>
              ) : (
                agents.map((a) => (
                  <div
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="truncate text-xs text-muted-foreground">
                        {activity.running(a.slug)
                          ? 'Updating…'
                          : activity.failure(a)
                            ? `Failed: ${activity.failure(a)}`
                            : a.lastRunAt
                    ...
                    <button
                      type="button"
                      title={activity.running(a.slug) ? 'Updating…' : 'Run now'}
                      aria-label={`Update ${a.name}`}
                      disabled={activity.running(a.slug)}
                      onClick={() => void activity.run(a.slug)}
                      className="rounded-md p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground"
                    >
                    ...
                    <button
                      ...
                      aria-label={`Automatic updates for ${a.name}`}
                    ...
                    >
                      <span
                        className={`absolute top-0.5 size-4 rounded-full bg-background shadow transition-all ${a.active ? 'left-[18px]' : 'left-0.5'}`}
                      />
                    </button>
                  </div>
                ))
            ...
            <section className="space-y-1.5">
              ...
              {manifest && manifest.capabilities.length > 0 ? (
                ...
                <p className="text-muted-foreground">
                  None declared — this app can’t use tools, LLM, or the copilot.
                </p>
              )}
            </section>
            ...
            <section className="space-y-1.5">
              ...
              {app.kind === 'installed' && app.install ? (
                <>
                  <InfoRow
                    k="Installed from"
                    v={app.install.repo ?? app.install.sourceUrl ?? 'unknown'}
                    mono
                  />
                  <InfoRow
                    k="Installed"
                    v={new Date(app.install.installedAt).toLocaleString()}
                  />
                  {app.install.updatedAt && (
                    <InfoRow
                      k="Updated"
                      v={new Date(app.install.updatedAt).toLocaleString()}
                    />
                  )}
                </>
              ) : app.publish ? (
                <>
                  ...
                  <InfoRow
                    ...
                    v={app.publish.repo}
                    mono
                    link={`https://github.com/${app.publish.repo}`}
                  />
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {app.kind === 'installed' && app.install?.repo && (
                  <button
                    ...
                    disabled={busyAction !== null}
                    onClick={() =>
                      void (updateInfo?.updateAvailable
                        ? doUpdate()
                        : checkForUpdate())
                    }
                  ...
                  >
                    ...
                    {busyAction === 'check' || busyAction === 'update'
                      ? 'Working…'
                ...
                {app.kind === 'local' && (
                  ...
                  >
                    ...
                    {app.publish ? 'Publish update' : 'Publish'}
                  </button>
                )}
                {app.kind === 'installed' ? (
                  <button
