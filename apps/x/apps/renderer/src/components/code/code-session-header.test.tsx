...
it('offers terminal and persisted Harness opt-out beside the composer, without files or diffs', async () => {
  ...
  await waitFor(() => expect(invoke).toHaveBeenCalledWith('codeSession:update', { sessionId: 's1', patch: { codeModeEnabled: false } }))
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Harness' }))
  await waitFor(() => expect(invoke).toHaveBeenCalledWith('codeSession:update', { sessionId: 's1', patch: { codeModeEnabled: true } }))
})
