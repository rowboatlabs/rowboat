...
- **Known gap to respect:** `spaces:uploadBlob`'s file-path fast-path reads the file on the machine where core runs (`spaces-deps.ts:178-181`) — wrong machine in remote mode. Whiteboard image paste should send bytes over the channel (pasted images are in-memory anyway); only large drag-dropped files care.
...
- Shared: `packages/shared/src/spaces.ts` (payload vocabulary `scene`/`scene_request`/`files`/`cursor`/`idle`, `WHITEBOARD_DIR` conventions), `ipc.ts` (`spaces:whiteboard` channel)
- Main: `apps/main/src/spaces/ipc.ts`; Server app: `apps/server/src/spaces-deps.ts` + `channels.ts`
- Renderer: `components/spaces/whiteboard-pane.tsx` (the collab loop — diff broadcasts gated on `getSceneVersion`, 20s full-scene self-heal, `reconcileElements` + `CaptureUpdateAction.NEVER` on receive, cursor frames at 33ms, collaborator TTL + heartbeats, text/blob snapshot save with conflict-reconcile-retry, image sync via space blobs), `spaces-view.tsx` (header Board button, ⌘4, full-bleed dispatch), `space-rail.tsx` (Whiteboards section; boards hidden from the file tree), `lib/spaces-selection.ts` (`whiteboard` kind), `vite.config.ts` (self-hosted Excalidraw fonts for dev + the packaged `app://` origin)
...
**Manual QA still to do (needs two running apps):** two-machine draw session (cursors, concurrent edits, join-mid-session), image paste propagation, packaged-build font rendering (`Excalifont` vs fallback — the CDN fallback must never fire), agent drawing via MCP (`propose_change` on `whiteboards/*.excalidraw` with valid `version`/`versionNonce`/`index` fields appearing live), remote-server-mode cursor latency feel.
