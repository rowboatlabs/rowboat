...
- Renderer: `components/spaces/whiteboard-pane.tsx` (the collab loop — diff broadcasts gated on `getSceneVersion`, 20s full-scene self-heal, `reconcileElements` + `CaptureUpdateAction.NEVER` on receive, cursor frames at 33ms, collaborator TTL + heartbeats, text/blob snapshot save with conflict-reconcile-retry, image tool disabled), `spaces/whiteboard.css` (the Rowboat skin — see note 2), `spaces-view.tsx` (header Board button, ⌘4, full-bleed dispatch), `space-rail.tsx` (Whiteboards section; boards hidden from the file tree), `lib/spaces-selection.ts` (`whiteboard` kind), `vite.config.ts` (self-hosted Excalidraw fonts for dev + the packaged `app://` origin)
