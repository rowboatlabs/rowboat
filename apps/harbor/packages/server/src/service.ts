...
export class HarborService {
  ...
  private toAsset(a: AssetRecord): Asset {
    return {
      id: a.id,
      path: a.path,
      version: a.version,
  ...
  private async requireAsset(spaceId: string, assetId: string): Promise<AssetRecord> {
    const asset = await this.store.getAssetById(spaceId, assetId);
  ...
  private async requireLiveAsset(spaceId: string, assetId: string): Promise<AssetRecord> {
  ...
  async createAsset(ctx: ActorCtx, spaceId: string, input: CreateAsset): Promise<CreateAssetResult> {
    ...
    return this.locked(spaceId, async () => {
      ...
      const record: AssetRecord = { id: this.ulid(), path: input.path, version: 1, updatedAt: this.now(), state: 'live' };
      await this.store.createAsset(spaceId, record);
  ...
  async moveAsset(
    ...
    return this.locked(spaceId, async () => {
      ...
      const at = this.now();
      await this.store.setAssetPath(spaceId, asset.id, input.toPath, at);
  ...
  async restoreAsset(
    ...
    return this.locked(spaceId, async () => {
      ...
      if (await this.store.getLiveAssetByPath(spaceId, dead.path)) {
  ...
  private async commit(
    ...
    await this.store.putAssetVersion(spaceId, asset.id, version, data, at);
    await this.store.appendChangeSet(changeSet);
  ...
  async assetHistory(
    ...
    if (opts.assetId !== undefined && !(await this.store.getAssetById(spaceId, opts.assetId))) return [];
  ...
  async createTopic(
    ...
    return this.locked(spaceId, async () => {
      ...
      const document = input.documentAssetId !== undefined ? await this.requireLiveAsset(spaceId, input.documentAssetId) : undefined;
    ...
    return this.locked(spaceId, async () => {
      ...
      switch (action.action) {
        ...
        case 'attach_document': {
          const asset = await this.requireLiveAsset(spaceId, action.assetId);
  ...
  async search(
    ...
    if (kinds.has('assets')) {
      ...
      results.assets = rows.slice(0, limit).map(({ record, snippet }) => {
        const { state: _live, ...asset } = this.toAsset(record);
